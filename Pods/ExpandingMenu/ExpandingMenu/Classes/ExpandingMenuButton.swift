...
open class ExpandingMenuButton: UIView, UIGestureRecognizerDelegate {
    ...
    fileprivate func makeEndPoint(_ itemExpandRadius: CGFloat, angle: CGFloat) -> CGPoint {
        ...
        case .top:
            return CGPoint(
                x: self.centerButton.center.x + CGFloat(cosf((Float(angle) + 1.0) * Float(Double.pi))) * itemExpandRadius,
                y: self.centerButton.center.y + CGFloat(sinf((Float(angle) + 1.0) * Float(Double.pi))) * itemExpandRadius
     //           y: self.centerButton.center.y + CGFloat(sinf((Float(angle) + 1.0) * Float(M_PI))) * itemExpandRadius
            )
        case .bottom:
            return CGPoint(
                x: self.centerButton.center.x + CGFloat(cosf(Float(angle) * Float(Double.pi))) * itemExpandRadius,
                y: self.centerButton.center.y + CGFloat(sinf(Float(angle) * Float(Double.pi))) * itemExpandRadius
            )
        }
    }
    ...
    fileprivate func expandMenuItems() {
        ...
        if self.enabledExpandingAnimations.contains(.MenuButtonRotation) == true {
            UIView.animate(withDuration: 0.1575, animations: { () -> Void in
                self.centerButton.transform = CGAffineTransform(rotationAngle: CGFloat(-0.5 * Double.pi))
            })
        } else {
            self.centerButton.transform = CGAffineTransform(rotationAngle: CGFloat(-0.5 * Double.pi))
        }
